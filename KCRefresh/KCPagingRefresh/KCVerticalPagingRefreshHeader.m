...
@interface KCVerticalPagingRefreshHeader()
...
@implementation KCVerticalPagingRefreshHeader
...
//    [super stateChange:state percent:percent];//注意这里不能调用supper的方法
    if (state==KCRefreshStateIdle) {
        ...
        if (state == KCRefreshStateRefreshing) {//避免弹回
            self.scrollView.contentInset=UIEdgeInsetsMake(self.originEdgeInsets.top + kKCRefreshComponentHeight*percent, self.originEdgeInsets.left, self.originEdgeInsets.bottom,self.originEdgeInsets.right);
            [self executeBlock];
        }
